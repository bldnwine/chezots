#!/usr/bin/env bash
# Toggle screen recording: desktop audio + mic (merged if available), no webcam
...
if screenrecording_active; then
  ...
  if [[ -n "$mic" && "$mic" != "null" && "$mic" != "$monitor" ]]; then
    audio_arg="${audio_arg}|${mic}"
  fi

  gpu-screen-recorder -w screen -f 60 -c mp4 ${audio_arg} -o "$filename" & disown
  sleep 1.5
  if screenrecording_active; then
    notify-send "Recording started (desktop + mic)" -t 1500
    pkill -RTMIN+8 waybar
  else
