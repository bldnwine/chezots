...
-- down over whatever workspace you are on. Its bindings live in hyprland.lua,
-- and its slide is animated below.
...
-- Seed the console with kitty via uwsm-app the first time it opens, rather than
-- at boot, so nothing is running until it is wanted.
local terminal = "uwsm-app -- kitty"
local seed = "[workspace special:scratchpad silent] " .. terminal
...
local function fit(monitor)
	...
	local reserved = monitor.reserved or { top = 0, bottom = 0, left = 0, right = 0 }
	height = height / monitor.scale - (reserved.top or 0) - (reserved.bottom or 0)
	width = width / monitor.scale - (reserved.left or 0) - (reserved.right or 0)

	local tall = math.floor(height * share)
	local wide = width
	if box and alone() then
...
-- off (SUPER+SHIFT+D / SUPER+SHIFT+1), and floated or tiled (SUPER+V).
